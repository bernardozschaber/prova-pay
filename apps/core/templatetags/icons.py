"""
{% icon "name" %} renders an inline Lucide icon (https://lucide.dev, ISC).
Keeping the paths here avoids a JavaScript icon bundle.
"""
...
def icon(name: str, size: int = 16, css_class: str = "") -> str:
    ...
    if paths is None:
        return ""
    return format_html(
        '<svg xmlns="http://www.w3.org/2000/svg" width="{0}" height="{0}" viewBox="0 0 24 24" fill="none" '
        'stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" '
        'class="icon {1}" aria-hidden="true">{2}</svg>',
        size,
        css_class,
        mark_safe(paths),
    )
